...
<template>
  ...
  <div class="relative min-h-screen w-full bg-[#051c12] flex items-center justify-center overflow-hidden font-['Cinzel'] text-amber-200">
    ...
    <!-- 1. ФОНОВЫЕ ЭФФЕКТЫ (Туман и символы) -->
      ...
      <div v-if="isSettingsOpen" class="fixed inset-0 z-[100] flex items-center justify-center p-6">
        ...
        <div class="relative w-full max-w-sm bg-[#0a2e22] border-2 border-amber-600/50 p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          <div class="space-y-8">
            <!-- Музыка -->
            <div class="space-y-3">
              <div class="flex justify-between text-xs uppercase tracking-[0.2em] text-amber-600">
                <span>Музыка Ветра</span>
          ...
          <!-- Кнопка Закрыть -->
          <button @click="toggleSettings" class="mt-10 w-full py-3 border border-amber-600/30 text-amber-500 hover:bg-amber-500 hover:text-[#051c12] transition-all duration-300 font-bold tracking-widest uppercase text-sm">
            Вернуться в туман
          </button>
        </div>
      </div>
...
.mythic-range::-webkit-slider-thumb {
  -webkit-appearance: none;
  width: 18px;
  height: 18px;
  background: #f59e0b;
  border: 2px solid #0a2e22;
  cursor: pointer;
...
.floating-symbol {
  bottom: -50px;
  animation: float linear infinite 15s;
  opacity: 0.1;
}
...
.swamp-mist {
  ...
  background: radial-gradient(circle, rgba(20, 83, 45, 0.2) 0%, transparent 70%);
  animation: rotate-mist 40s linear infinite;
}
